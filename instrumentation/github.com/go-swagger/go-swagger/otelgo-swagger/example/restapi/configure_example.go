...
import (
	...
	"go.opentelemetry.io/opentelemetry-go-contrib/instrumentation/github.com/go-swagger/go-swagger/otelgo-swagger/example/restapi/operations"
)

//go:generate swagger generate server --target ../../example --name Example --spec ../swagger.yml --principal interface{}
...
func configureAPI(api *operations.ExampleAPI) http.Handler {
	...
	api.GetUsersUserIDHandler = operations.GetUsersUserIDHandlerFunc(func(params operations.GetUsersUserIDParams) middleware.Responder {
		if params.UserID == 123 {
			return operations.NewGetUsersUserIDOK().WithPayload(&models.User{
				ID:   123,
				Name: "test user",
			})
		}
